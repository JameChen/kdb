...
import com.nahuo.kdb.dialog.DropDownPopupWindow;
import com.nahuo.kdb.model.SelectBean;
import com.nahuo.kdb.tools.KeyBoardTool;
import com.nahuo.kdb.tools.SizeTool;
import com.nahuo.kdb.tools.StringTool;
...
private  Context mContext;
    ...
    public  String NUSELETED_SHOW_NAME = "请选择";
    ...
    boolean canSelect;
    public void setupDataList(List<SelectBean> dataList) {
...
//        }
     ...
     * @return
     */
    private int dealItemMaxWidth(List<SelectBean> dataList,int btnWidth,float textSize) {
        int tempWidth=0;
        int maxLen=0;
        String maxName="";
        for (int i = 0; i < dataList.size(); i++) {
            String nowName=dataList.get(i).getName().toString();
            if (nowName.length()>maxLen){
                maxLen=nowName.length();
                maxName=nowName;
            }
        }
        if (maxLen>NUSELETED_SHOW_NAME.length()){
            int defaultTextWidth= StringTool.getAllTextWidth(NUSELETED_SHOW_NAME,textSize);
    ...
    public void onClick(View v) {
        ...
        if (!ListUtils.isEmpty(dataList)){
            DropDownPopupWindow myPopupwindow=new DropDownPopupWindow(mContext,dataList,Type,0,NUSELETED_SHOW_NAME);
            ...
            myPopupwindow.setOnItemSelectListener(new DropDownPopupWindow.OnItemSelectListener() {
                @Override
                public void onItemSelect(SelectBean map,int pos,int realPos,int Type) {
                    if (map==null){
                        ((DropDownView) nowClickView).setText(NUSELETED_SHOW_NAME);
                    }else{
                        ////
                        ((DropDownView) nowClickView).setText(map.getName());
...
//                    }
                    ...
                    if (onItemClickListener!=null) {
                        onItemClickListener.onItemClick(map,pos,realPos,Type);
                    }
                }
            });
        }
...
//        }
    ...
    public  interface OnItemClickListener{
        void  onItemClick(SelectBean map, int pos, int realPos,int Type);
    }
