...
import com.nahuo.kdb.adapter.AchieveAdapter;
...
public class AchievementActivity extends AppCompatActivity  implements View.OnClickListener, DropDownView.OnItemClickListener{

    public static String TAG = AchievementActivity.class.getSimpleName();
    private AchievementActivity vThis = this;
    ...
    private List<AchieveBean.SellerListBean> data = new ArrayList<>();
...
//        titlebar_icon_right.setVisibility(View.GONE);
        ...
        adapter = new AchieveAdapter(vThis);
    ...
    public class LoadDataTask extends AsyncTask<Void, Void, Object> {
        ...
        protected void onPostExecute(Object object) {
            ...
            if (object instanceof String && ((String) object).startsWith(ERROR_PREFIX)) {
                ...
                return;
            } else if (object instanceof AchieveBean) {
                AchieveBean bean = (AchieveBean) object;
                if (bean != null) {
                    tv_summary.setVisibility(View.VISIBLE);
                    tv_summary.setText("销售数量：" + bean.getSalesTotalQty() + "件   " + "客单价：" + FunctionHelper.DoubleFormat(bean.getPerCustTran())
                            + "   金额" + FunctionHelper.DoubleFormat(bean.getTotalAmount()) );
                    //                SellerUsers.clear();
                    if (mIsRefresh) {
                        if (data != null)
                            data.clear();
                        if (!ListUtils.isEmpty(bean.getSellerList()))
                            data.addAll(bean.getSellerList());
                    } else {
                        if (!ListUtils.isEmpty(bean.getSellerList()))
                            data.addAll(bean.getSellerList());
                    }
...
//                    }
        ...
        protected Object doInBackground(Void... params) {
            try {
                AchieveBean saleBean = OtherAPI.getPerformancePK(vThis, mPageIndex, mPageSize, beginTime, endTime, itemCode, keyword);
